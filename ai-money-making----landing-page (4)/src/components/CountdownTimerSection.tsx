...
export const CountdownTimerSection: React.FC<CountdownTimerSectionProps> = ({ onRegisterClick }) => {
  // Target date: This Thursday at 20:00:00 (8:00 PM) IST or dynamic fallback
  ...
  useEffect(() => {
    const calculateTimeLeft = () => {
      // Calculate target to This Thursday 8:00 PM
      const now = new Date();
      const currentDay = now.getDay(); // 0 = Sun, 1 = Mon, ..., 4 = Thu
      let daysUntilThursday = (4 - currentDay + 7) % 7;
      let targetDate = new Date(now.getFullYear(), now.getMonth(), now.getDate() + daysUntilThursday, 20, 0, 0);

      // If today is Thursday and past 8:00 PM, target next Thursday
      if (daysUntilThursday === 0 && now.getTime() >= targetDate.getTime()) {
        targetDate = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 7, 20, 0, 0);
      }

      let diff = targetDate.getTime() - now.getTime();

      if (diff <= 0) {
        // Fallback realistic active urgency loop
        diff = (2 * 24 * 3600 + 6 * 3600 + 45 * 60 + 30) * 1000;
      }
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-6 lg:gap-8">
          ...
          <div className="text-center lg:text-left space-y-2 max-w-md">
            ...
            <h3 className="text-2xl sm:text-3xl font-black text-white tracking-tight font-heading">
              Session Starts on <span className="text-[#f5a623]">This Thursday @ 08:00 PM</span>
            </h3>
